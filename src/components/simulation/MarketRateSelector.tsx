...
export function MarketRateSelector({
  ...
  return <div className="space-y-2">
      ...
      {selectedType === "ipca_plus" && <div className="animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex-wrap gap-2 p-3 rounded-lg bg-muted/50 border items-center justify-center flex flex-row px-px py-[5px]">
            <div className="items-center gap-2 flex flex-row">
              <span className="text-sm text-muted-foreground">IPCA ({rates.ipca}%) +</span>
              <div className="gap-1 items-center justify-center flex flex-row">
                <Input type="number" step="0.5" min="0" max="20" placeholder="5" value={ipcaSpread} onChange={handleSpreadChange} className="h-8 w-16 text-sm text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
      ...
      {selectedType === "custom" && <div className="relative animate-in fade-in slide-in-from-top-2 duration-200">
          <Input id={id} type="number" step="0.01" placeholder="Digite a taxa desejada" value={customValue} onChange={handleCustomChange} className={cn("pr-8", error ? "border-destructive" : "")} />
